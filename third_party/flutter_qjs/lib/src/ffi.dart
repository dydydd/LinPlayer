...
// Dart 3 / ffi 2.x 兼容补丁：原生回调（Pointer.fromFunction）返回类型必须为
// 非空 Pointer。原版返回 Pointer<JSValue>?（可空）在 Dart 3.x 下编译报错。
// 这里改为返回非空 Pointer<JSValue>，查不到 runtime 时返回 nullptr，并给
// Pointer.fromFunction 显式指定原生签名类型参数。
...
Pointer<JSRuntime> jsNewRuntime(
  ...
  final rt = _jsNewRuntime(
      Pointer.fromFunction<_JSChannelNative>(channelDispacher), timeout);
