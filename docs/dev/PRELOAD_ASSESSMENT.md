...
- “预加载完再点播放，开始播放前还是要等一段时间”：现在更准确地说，主要卡在播放页自己的前半段准备流程，尤其是 playback source 包装、本地代理注册、播放器初始化、首个探测请求。
- “等待过程中仍会补少量缓存，然后才真正开始播放”：这说明首帧前仍存在少量 cache fill / probe / tail 补齐，预热窗口还没有完全覆盖播放器真正的首个入场请求。
- “一旦进入加载到播放那段，看不到预加载前缀再次灌入缓存流量”：这说明后半段已经有改善，共享缓存复用和前缀不重复回源基本成立，问题主要集中在点击播放到首帧之间的前半段。
...
- 但进入播放页后仍然会继续执行本地续播读取、播放源包装、播放器初始化，以及首个探测请求对应的少量补齐，因此真实体感收益仍需要手工回归验证。
...
2026-04-14 构建后手测得到的现象是：

- 预加载完成后再点击播放，开始播放前仍然要等待一段时间。
- 这段等待里仍会补少量缓存，然后才真正开始播放。
- 但从“加载中”到“开始播放”的那一段，已经基本看不到预加载前缀再次进入缓存流量。

这组现象说明：

- “后半段”已经开始改善：共享缓存前缀复用成立，播放开始后的继续补流明显减少。
- “前半段”仍未改善：点击播放到首帧之间的 page / proxy / player 准备链路仍在关键路径。
- 现在更应该把“进入播放慢”拆成两个问题分别看：
  1. 点击播放到首帧前的准备等待
  2. 开始播放后是否还会重复拉前缀、继续明显缓冲
...
1. 把“点击播放到首帧”拆成明确的阶段耗时：本地续播读取、`_buildPlaybackSource()`、proxy wrap、player initialize、首请求 / 首探测缓存命中。
2. 继续把 handoff 之后的播放页准备链路收口到更接近最终播放对象，而不是只停留在共享 `ResolvedPlaybackSource`。
