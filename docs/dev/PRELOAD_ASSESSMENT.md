...
- 当前代码已先把播放页 startup preload barrier 的同步等待撤回为后台 warmup 观测日志，不再阻塞 `controller.initialize()`；但最新复测说明，**这一步并没有恢复 `EXO` 基线**，`EXO` 仍然会出现“只加载、不播放”。
- 同样地，`MPV` 最新复测里前半段仍在加载和缓存大量内容，现阶段更应优先怀疑**首帧前主链路并没有稳定复用预加载结果**，而不只是“命中缓存后消费偏慢”。
- 换句话说，目前能确认成立的仍然只是“共享缓存与 handoff 基础设施有价值”；但“用户点击播放就更快”不仅没有被证明，最新尝试还在 `EXO` 路径上给出了反例。

用户最新手测反馈中的现象，现在需要明确拆成两个内核维度来看：

- `MPV`：最新复测里前半段仍然需要明显加载和缓存大量内容，问题本质已更接近“首帧前疑似没有真正吃到预加载结果”，而不只是 playback fill 过重。
- `EXO`：即使撤回了 startup preload barrier 的同步等待，最新复测里仍然只加载不播放；因此当前矛盾已从“缓存消费偏慢”升级为“播放入口基线仍未恢复”。
- 两个内核不再能被写成“只是同一问题的不同轻重程度”；当前更准确的写法应是：`MPV` 仍未解决前半段重缓存，`EXO` 则已经进入阻塞级回归。

一句话概括：

- 现在的预加载仍可被视为“共享缓存预热能力已经有基础设施”。
- 当前工作树对应的最新体感**仍不能被视为可接受状态**：`MPV` 前半段仍在加载和缓存大量内容，`EXO` 仍然只加载不播放；即使已撤回播放页 startup preload barrier 的同步等待，基线问题也没有真正恢复。
...
- 作为“用户体感上的预加载直开能力”：`1/10`
- 作为“播放器端到端统一预加载基础设施”：`2/10`
...
- 但最新复测已经说明：撤回同步等待后，`EXO` 仍然没有恢复到可正常开始播放的基线。
- `MPV` 当前已不能只表述为“预加载完成后仍有偏重的继续缓存 / playback fill”；更保守且更符合现象的写法是：前半段**疑似没有真正复用预加载结果**。
- `EXO` 当前也不应继续表述为“只是命中缓存后消费偏慢”，而应明确表述为“播放入口基线仍未恢复，现象为只加载不播放”。
...
### 9.1.3 2026-04-14 最新复测说明：撤回同步等待后，EXO 仍未恢复，MPV 仍疑似未命中预加载

2026-04-14 同日最新复测现象，需要按内核拆开记录：

- `MPV` 内核下，预加载完成后点击播放，前半段仍然持续加载并缓存大量内容；从体感上看，这更像“首帧前仍在重新走一段重加载链路”，而不只是残余的少量补流。
- `EXO` 内核下，即使撤回了 startup preload barrier 的同步等待，最新复测里仍然只加载不播放；说明此前“撤回同步等待后应先恢复基线”的预期并没有成立。
- 因为 `EXO` 连真正开始播放都没有恢复，所以此前那种“代码已止血、接下来继续微调前半段”的描述，在当前最新状态上也不再成立。

这组现象说明：

- 把 preload barrier 或额外 warmup 等待放进播放页关键路径，本身就是高风险动作；它可能把原本只是“慢”的问题放大成“进页阻塞 / 假死”。
- `MPV` 现阶段也仍然没有证据表明“调轻 loopback 快启动缓存档位”足以解决问题；更优先的诊断方向应该是：播放器首个关键请求是否真的命中了 `PreparedPlaybackPreload` 对应的 `cacheKey / proxy URL / start window`。
- `EXO` 现阶段的首要任务已经不是“继续优化命中缓存后的消费速度”，而是先找出为什么撤回同步等待后仍然会“只加载不播放”：是 `controller.initialize()`、首探测请求、代理链路，还是 handoff 后的播放对象并未真正与预加载结果对齐。

因此当前更合理的阶段判断是：

- “共享缓存 + handoff”这一层基础设施依然值得保留。
- “把 preload 变成播放入口关键路径上的同步等待”这一做法，至少在当前实现上已经被最新手测证明为不安全。
- 当前代码层面虽已撤回这段同步等待，命中 handoff 时的 current-item warmup 也已回到后台观测；但最新复测已经说明：`EXO` 仍未恢复到“可进页后真正开始播放”的基线，而 `MPV` 首帧前是否真正复用预加载也仍未被证明。
...
- 但当前最新体感已经不能再被描述为“虽然还慢，但方向正确”：`EXO` 仍然只加载不播放，`MPV` 前半段也仍然像没有真正复用预加载内容。

当前最准确的描述应该是：

> 预加载现在已经能做到“提前把一部分可播数据写进共享缓存”，但它仍然没有稳定做到“点击播放后明显更快”。  
> 更严重的是，当前最新复测说明：即使撤回了播放页 startup preload barrier 的同步等待，`EXO` 路径仍然只加载不播放；而 `MPV` 路径前半段仍然在加载和缓存大量内容，现阶段更应优先怀疑首帧前没有真正复用预加载结果。  
> 因此当前阶段的正确目标，已经不是继续把文档写成“后半段已改善、前半段继续微调”，而是先承认 `EXO` 基线恢复失败、`MPV` 真实复用仍未证明，再回到“播放对象是否与预加载结果真正对齐”的根因诊断。
...
2. 优先恢复 `EXO` 到“能进入播放页、能真正开始播放”的基线，再定位到底是哪一段让它停留在“只加载不播放”。
