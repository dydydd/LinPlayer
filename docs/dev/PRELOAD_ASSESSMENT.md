...
- 但“预加载完成后，点击播放就明显更快、且几乎不再等待首帧”这件事，**仍然没有成立**。
- 问题不再只是“有没有把结果带进播放页”，而是“带进播放页之后，播放器初始化、首个探测请求、少量 cache fill 这些准备成本还没有完全收口”。

用户最新手测反馈中的现象，现在需要明确拆成两个内核维度来看：

- `MPV`：预加载完成后再点击播放，真正开播前仍会继续缓存一段**明显偏大**的内容；这已经不只是“首帧前少量 cache fill”，而更像起播前仍有一段较重的继续缓存过程。
- `EXO`：当前片源已经恢复到可正常播放，不再以“直接播放报错”为主要矛盾；命中 handoff 时也已不再重复 `_buildPlaybackSource()` / proxy wrap，但“加载预加载缓存”本身仍明显偏慢，说明命中缓存后的消费链路还没有顺畅转化为首帧时延下降。
- 两个内核的共同点是：一旦进入“加载中 -> 开始播放”后半段，已经基本看不到预加载前缀再次大规模灌入缓存流量，共享缓存复用和前缀不重复回源基本成立。
- 因此当前剩余问题，已经不再适合笼统写成“预加载后起播还是慢”，而应该写成“`MPV` 首帧前继续缓存偏重，`EXO` 命中缓存后的加载消费链路偏慢”。

一句话概括：

- 现在的预加载更像“共享缓存预热能力已经落地”。
- 它已经开始改善“进入播放后的后半段”，但还不是“端到端点击播放就明显提速的秒开能力”。
...
- 但进入播放页后仍然会继续执行本地续播读取、播放器初始化，以及首个探测请求对应的补齐，因此真实体感收益仍需要手工回归验证。
- `MPV` 当前主要卡在“预加载完成后仍有偏重的继续缓存 / playback fill”；`EXO` 当前主要卡在“命中预加载缓存后到真正开始播放之间的加载消费链路仍偏慢”。
...
### 9.1.3 最新构建后手测说明：后半段有改善，前半段仍未收口

2026-04-14 最新构建后的手测现象，需要按内核拆开记录：

- `MPV` 内核下，预加载完成后点击播放，真正开始播放前仍会继续缓存明显偏大的一段内容；这已经不是“首帧前少量 cache fill”，而是更接近“仍有一段较重的继续缓存后才开播”。
- `EXO` 内核下，当前片源已经恢复到可正常播放，不再以“直接播放报错”为主要矛盾。
- 但 `EXO` 在命中预加载后，“加载预加载缓存”本身仍明显偏慢；这条路径现在更偏向 `controller.initialize()`、首探测请求和缓存消费节奏仍未顺畅收口，而不再是稳定重复 `_buildPlaybackSource()` / proxy wrap。
- 两个内核的共同点是：从“加载中”到“开始播放”的后半段，已经基本看不到预加载前缀再次进入缓存流量。

这组现象说明：

- “后半段”已经开始改善：共享缓存前缀复用成立，播放开始后的继续补流明显减少。
- `MPV` 的主要剩余矛盾，是首帧前仍有偏重的继续缓存 / playback fill。
- `EXO` 的主要剩余矛盾，是命中缓存后到控制器真正开始播放之间的加载消费链路仍偏慢。
- 现在不应该再把“进入播放慢”视为一个单一问题，而应该至少拆成：
  1. `MPV` 首帧前继续缓存为什么仍然偏重
  2. `EXO` 命中预加载缓存后为什么消费仍慢

而当前进展更接近：

- “后半段前缀不重复回源”这一类问题已有明显改善
- 内核分化后的前半段问题，已经成为新的主要矛盾
...
- `EXO` 命中 handoff 时已不再重复 `_buildPlaybackSource()` / proxy wrap，普通 `GET` 也已能复用 in-flight cache fill
- `MPV` 在预加载完成后仍会先继续缓存明显偏大的内容才真正开播
- `EXO` 虽然已恢复到可正常播放，但“加载预加载缓存”本身仍明显偏慢
- HLS / 续播 / 探测请求仍可能让代理继续走 `cached-prefix + remote-tail` 或出现 `range-not-covered`

因此，最准确的描述应该是：

> 预加载现在已经能做到“提前把一部分可播数据写进共享缓存”；  
> 但它还做不到“预加载完成后，详情页一点播放就明显更快且基本不再等待”。  
> 真正还缺的，不再是“有没有把预加载结果带过页面跳转”或“EXO 是否还在重复 `_buildPlaybackSource()` / proxy wrap”，而是把 handoff 之后剩余的播放器初始化、首探测请求与缓存消费链路继续收口，并分别解决 `MPV` 的首帧前重缓存与 `EXO` 的缓存消费偏慢。

---

如果后续准备继续重构，建议优先做三件事：

1. 把“点击播放到首帧”拆成明确的阶段耗时，并按 `MPV` / `EXO` 分开记录：本地续播读取、player initialize、首请求 / 首探测缓存命中、少量 cache fill。
2. 优先定位 `MPV` 首帧前那段明显偏重的继续缓存究竟由谁触发，是播放器自身策略、loopback proxy `playbackFill`，还是首请求保护性补齐。
3. 优先定位 `EXO` “加载预加载缓存”偏慢到底耗在 `controller.initialize()`、首个探测请求，还是命中缓存后的实际消费链路，并继续用真实首请求范围校正预热窗口。
