...
- 问题不在“有没有触发预加载”，而在“预加载结果只收口到了缓存层，还没有收口到进入播放页后的准备链路”。
...
- 缓存层、代理层、去重层已经做了不少实事。
- 但详情页 / 下一集预加载结果并没有被直接带进播放页；进入播放页后仍然会重新执行 `_buildResolvedPlaybackSource()`、重新取远端/本地续播进度、重新包装播放源、重新初始化播放器。
...
### 9.1.1 详情页预加载结果没有被直接带进播放页

这次复核后，最需要单独点出来的是：**路由跳转时没有把 prepared result 传下去**。

当前代码里，详情页 / 剧集页预加载完成后，跳转到播放页时传递的仍然只是：
...
- 已构建完成的 `PlaybackSourceBuildResult`
- 已确定的 `ResolvedPlaybackSource`
- 已生成好的 loopback proxy URL / cache snapshot

相反，播放页初始化仍会重新执行一整段准备链路：
...
- 详情页已经跑完预加载，不等于播放页可以“直接拿现成结果开播”
- 当前缺的不是单纯的缓存命中，而是“页面跳转后的 prepared result handoff”
- 这正是“预加载完成后进入播放器仍然慢”的第一来源
...
- 没有把详情页 / 下一集阶段已准备好的结果直接带进播放页
- 播放页进入后仍会重新执行 source build、续播点拉取、播放器初始化等准备动作
...
> 真正缺的不是再多下一点，而是把预加载结果带过页面跳转，并收口播放页自己的准备链路。
