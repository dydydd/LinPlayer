...
- 问题不再只是“有没有把结果带进播放页”，而是“带进播放页之后，`_buildPlaybackSource()`、本地代理包装、播放器初始化、首个探测请求这些准备成本还没有完全收口”。

用户最新手测反馈中的现象，现在需要明确拆成两个内核维度来看：

- `MPV`：预加载完成后再点击播放，真正开播前仍会继续缓存一段**明显偏大**的内容；这已经不只是“首帧前少量 cache fill”，而更像起播前仍有一段较重的继续缓存过程。
- `EXO`：当前片源已经恢复到可正常播放，不再以“直接播放报错”为主要矛盾；但“加载预加载缓存”本身仍明显偏慢，说明命中缓存后的消费链路还没有顺畅转化为首帧时延下降。
...
- 但进入播放页后仍然会继续执行本地续播读取、播放源包装、播放器初始化，以及首个探测请求对应的补齐，因此真实体感收益仍需要手工回归验证。
...
- 但这并没有消除首帧前的等待，因为 `_buildPlaybackSource()`、loopback proxy 注册、播放器初始化和首个探测请求仍在关键路径。
...
- 但 `EXO` 在命中预加载后，“加载预加载缓存”本身仍明显偏慢，说明缓存命中后的消费链路和播放器准备链路仍未顺畅收口。
...
> 真正还缺的，不再是“有没有把预加载结果带过页面跳转”，而是把 handoff 之后的播放页准备链路继续收口，并分别解决 `MPV` 的首帧前重缓存与 `EXO` 的缓存消费偏慢。

---

如果后续准备继续重构，建议优先做三件事：

1. 把“点击播放到首帧”拆成明确的阶段耗时，并按 `MPV` / `EXO` 分开记录：本地续播读取、`_buildPlaybackSource()`、proxy wrap、player initialize、首请求 / 首探测缓存命中。
