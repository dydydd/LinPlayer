# 预加载问题复盘与下一阶段任务清单
> 更新日期：2026-04-01
>
> 配套文档：`docs/dev/PRELOAD_ASSESSMENT.md`

## 1. 当前结论

当前“预加载”已经不是空功能，但它的本质仍然是：

- 先按最终播放源做一轮网络预热
- 尝试把首段字节灌进本地 loopback 代理缓存
- 期望播放器后续请求能命中这部分缓存

这条链路在少数场景下可能有收益，但它**还不等于真正的播放器缓冲复用**。

用户当前实测现象已经很明确：

- 电影、剧集进入播放后依然要等待加载
- 等预加载完全跑完再点播放，依然会重新跑一次流量
- 体感上并没有达到“直接播放”的效果

这说明我们当前方案的主要问题已经不是“有没有触发预加载”，而是：

> 预加载拿到的字节，并没有被 MPV / EXO 以稳定、可靠、可复用的方式直接消费。

## 2. 为什么现在还是会重复加载

根因已经基本确认：

- MPV 当前仍然是 `player.open(Media(url))`，本质上还是给播放器一个 URL，让它自己重新开流。
- EXO 当前仍然是 `VideoPlayerController.networkUrl(...)`，同样是播放器自己按 URL 发请求。
- 我们现有的 loopback 代理缓存，只能在“播放器首个请求的 URL / Header / Range 恰好命中我们提前灌进去的缓存范围”时复用。
- 一旦播放器实际请求模式和预热范围不一致，就会继续回源，导致重复流量和重新等待。

换句话说，现在的问题不是“预加载太少”，而是：

> 我们做的是“网络预热”，不是“播放器直接消费同一份本地缓存数据”。

## 3. 直接把预加载流怼进去播放，当前为什么做不到

从现在的播放器接入方式看，直接把一段已经下载到内存里的视频字节“塞给播放器边播边续”，并不现实：

- MPV 目前接的是 URL / file 路径，不是我们可持续写入的自定义内存流。
- EXO 当前 Flutter 接入层也是 URL / file / contentUri 这一类数据源，不是一个我们自己掌控的实时字节流接口。
- `Media.memory` 这类能力更接近“一次性已有数据落临时文件”，不适合持续增长的网络视频流。

所以这件事难，不是因为“预加载功能不好写”，而是因为：

> 要想真正复用，必须把播放器入口从“远端 URL”改造成“本地缓存代理 / 本地文件 / 可按需补齐的数据源”。

## 4. 方案决策

结论已经很清楚，下一阶段不应继续在“首段内存缓存命中率”上做小修小补，而要切换到新的主方案：

> 从“网络预热”升级为“磁盘落地的边下边播缓存”。

新方案目标：

- 预加载先把视频数据写入本地缓存
- 播放时优先消费同一份本地缓存
- 缓存缺哪一段，就由本地缓存代理继续回源补哪一段
- 预加载和播放共享同一个缓存 key、同一个下载状态、同一份字节数据

这样才能真正做到：

- 不重复下载已经拿到的前缀数据
- 等预加载跑完后，进入播放页能明显更快起播
- 甚至可以实现“先播已缓存部分，后面边下边补”

## 5. 本阶段的设计边界

### 5.1 本阶段目标

- 把“当前片源预加载”升级成“当前片源本地缓存预热”
- 让 MPV / EXO 的实际播放优先走同一份本地缓存代理
- 让“预加载完成后再进播放”不再重复跑前缀流量
- 保持现有 `ResolvedPlaybackSource` / `PlaybackSourceBuilder` 主干不回退

### 5.2 本阶段不做

- 不在这一轮追求 Web 支持
- 不在第一阶段就把 HLS ABR 做到完美
- 不在第一阶段重写全部播放器接入层
- 不在第一阶段做下载器产品化 UI

## 6. 已确认可保留的成果

这些已经做过的工作仍然有效，不需要推倒重来：

- [x] `PlaybackSourceBuilder` / `ResolvedPlaybackSource` 已经统一了大部分播放源决策
- [x] 详情页、MPV 播放页、EXO 播放页都已经接入统一预加载入口
- [x] 当前片源 / 下一集 / 续播点预加载触发链路已经接通
- [x] loopback 代理已具备基础缓存、warmup、range 处理能力
- [x] MPV / EXO 的自动横竖屏和自动起播问题已单独修正

这些工作会作为“本地缓存代理方案”的上游基础继续复用。

## 7. 下一阶段任务清单

## Phase A：补全事实诊断

目标：把“为什么没有复用”从主观体感变成可定位日志。

- [x] 给 MPV / EXO 首次播放请求增加诊断日志
- [x] 记录播放器首个请求的 URL、Range、Header 摘要
- [x] 记录 loopback 代理命中情况
- [x] 记录命中失败原因
- [x] 区分“缓存不存在 / 范围不覆盖 / Header 不一致 / 被直接回源”
- [x] 在诊断报告里加入“预加载缓存复用结果”摘要

验收标准：

- 能明确回答“预加载完成后为什么还是重新跑流量”
- 能区分是播放器请求模式问题，还是缓存覆盖范围问题

## Phase B：定义磁盘缓存模型

目标：为“边下边播”建立稳定的数据结构。

- [x] 设计统一 `CacheKey`
- [x] `CacheKey` 至少纳入：
  - [x] 最终远端 URL 指纹
  - [x] 必要请求头指纹
  - [x] `mediaSourceId`
  - [x] 音轨 / 字幕选择
  - [x] 代理语义
- [x] 设计缓存目录结构
- [x] 设计元数据文件
- [x] 设计已完成字节区间描述
- [x] 设计缓存状态
  - [x] warming
  - [x] playable
  - [x] completed
  - [x] failed
  - [x] stale
- [x] 设计 TTL、清理、淘汰策略

验收标准：

- 同一片源的预加载和播放能稳定映射到同一个 `CacheKey`
- 能表达“前 8MB 已就绪、后续仍未完成”这种状态

## Phase C：实现磁盘写入下载器

目标：把预加载从“只读网络”改成“持续写入本地缓存”。

说明：本轮先采用“内聚在磁盘缓存代理里的薄下载入口”，而不是单独拆一个重型下载器服务。

- [x] 新建磁盘缓存下载器服务
- [x] 支持 HTTP Range 下载
- [x] 支持断点续写
- [x] 支持记录重定向后的最终 URL
- [x] 支持继承当前播放源的 Header / 代理配置
- [x] 支持并发去重
- [x] 支持下载进度通知
- [x] 支持失败重试和 scoped 熔断

验收标准：

- 能把当前片源前若干 MB 写入本地缓存文件
- 重复触发同一片源预加载时，不会重复下载同一段

## Phase D：实现本地缓存代理

目标：让播放器真正消费本地缓存，而不是碰运气命中内存前缀。

- [x] 新建“磁盘缓存代理”模式
- [x] 播放请求进入代理时，优先从磁盘缓存响应
- [x] 缓存缺失区间时，由代理按需拉远端并写回磁盘
- [x] 支持顺序播放
- [x] 支持显式 Range 请求
- [x] 支持 `bytes=start-end`
- [x] 支持 `bytes=start-`
- [x] 支持 HEAD / 元数据探测
- [x] 支持多请求并发读同一个缓存对象
- [x] 保证播放器只需要感知一个本地代理 URL

验收标准：

- 预加载完成前缀后，播放器首次请求前缀区间时不再重复回源
- 同一片源播放期间，后续缺失区间会被边播边补

## Phase E：播放链路切到统一缓存入口

目标：让 MPV / EXO 实际播放走统一缓存代理。

- [x] MPV 播放优先走本地缓存代理 URL
- [x] EXO 播放优先走本地缓存代理 URL
- [x] 当前片源预加载与实际播放共享同一个缓存对象
- [x] 去掉页面侧“等一下预加载”的临时策略
- [ ] 保持自动播放、自动横竖屏、续播逻辑不回退

验收标准：

- “等预加载完成后进入播放”时，首段不再重新下载
- MPV 与 EXO 的表现一致，不再出现一个内核复用、另一个不复用

## Phase F：续播点与下一集策略收口

目标：让缓存预热和真实观看路径一致。

- [x] 当前片源从 0 秒开播时预热前缀区间
- [x] 当前片源从续播点进入时预热续播区间
- [x] 下一集预加载沿用系列级版本偏好
- [x] 当前集与下一集缓存 key 隔离
- [x] 详情页预热和播放页兜底预热共享同一缓存状态

验收标准：

- 续播进入时，播放器优先命中续播点附近的缓存
- 下一集预加载不会误热到错误版本

## Phase G：HLS 专项

目标：把 direct file 方案跑通后，再处理 HLS 的真实难点。

- [x] 先明确第一阶段是否只保证 direct file / redirect file / body-link file
- [x] 为 HLS 单独设计缓存策略
- [x] 明确是缓存最终 rendition，还是缓存 master/segment 层
- [ ] 评估播放器实际所选 rendition 的可观测性
- [x] 评估 HLS 是否需要单独的本地清单重写方案

验收标准：

- direct file 方案先稳定落地
- HLS 不再混在主链路里模糊推进

## Phase H：测试与回归

目标：把这件事变成可持续维护能力。

- [x] 单测：`CacheKey` 稳定性
- [x] 单测：磁盘区间写入与读取
- [x] 单测：并发下载去重
- [x] 单测：代理范围请求命中
- [x] 单测：播放请求等待 in-flight 预加载后直接命中缓存
- [x] 集成测试：预加载完成后播放不重复下载前缀
- [x] 集成测试：播放中缺失区间边播边补
- [ ] 手工回归：电影首播
- [ ] 手工回归：剧集首播
- [ ] 手工回归：续播进入
- [ ] 手工回归：切集进入下一集
- [ ] 手工回归：MPV / EXO 双内核一致性

验收标准：

- 可以明确证明“流量没有重复跑前缀”
- 可以明确证明“等待预加载完成后再点播放，起播更快”

## 8. 推荐实施顺序

建议严格按下面顺序推进：

1. Phase A：先补诊断
2. Phase B：定缓存模型
3. Phase C：做磁盘下载器
4. Phase D：做磁盘缓存代理
5. Phase E：把 MPV / EXO 切过去
6. Phase F：补续播与下一集
7. Phase G：最后做 HLS
8. Phase H：补测试与手工回归

不建议再继续在“当前内存前缀缓存命中率”这条线上投入太多时间。

## 9. 第一优先级 TODO

如果接下来只能先做一件事，优先做：

- [x] 实现“磁盘缓存代理”最小可用版本，只覆盖 direct file / redirect file / body-link file

这是最有可能直接改变用户体感的一步，因为它能第一次真正做到：

- 预加载和播放共享同一份字节
- 不再依赖播放器首个 Range 恰好命中内存前缀
- 等预加载结束后进入播放，能直接利用已经落地的数据

## 10. 完成定义

当满足以下条件时，才算“预加载真正可用”：

- [ ] 等预加载完成后再进入播放，不会重复下载前缀数据
- [ ] 电影首播可明显缩短等待
- [ ] 剧集首播可明显缩短等待
- [ ] 续播点附近可明显缩短等待
- [ ] MPV / EXO 两个内核表现一致
- [x] 诊断日志能解释命中与未命中的原因

## 11. 最终判断

这件事确实难做，但难点已经不是“有没有思路”，而是：

> 现在必须从“网络预热优化”切换到“本地缓存代理架构”。

继续修现有方案，收益会越来越小；切到“磁盘落地的边下边播缓存”，才有机会真正解决你现在遇到的两个核心问题：

- 为什么预加载完了还是要等
- 为什么明明预加载过了还会重复跑流量
