...
class _ExoPlayNetworkPageState extends State<ExoPlayNetworkPage>
  ...
  bool _mobilePlaybackEndHandled = false;
  ...
  Future<void> _applyMobileLoopModeToExo() async {
    ...
    try {
      await c.setLooping(_mobileLoopMode != MobilePlaybackLoopMode.none);
    } catch (_) {}
  }
  ...
  Future<void> _onMobilePlaybackEnded() async {
  ...
  Widget _buildMobileVideoSurface(VideoPlayerController controller) {
    ...
    if (mode.usesFixedAspectRatio) {
      body = Center(
        child: AspectRatio(
          aspectRatio: mode.fixedAspectRatio!,
          child: body,
        ),
      );
    }
    body = ClipRect(child: SizedBox.expand(child: body));
    if (_mobileListenVideoOnly) {
      body = IgnorePointer(
        child: Opacity(
          opacity: 0,
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Builder(
                                      builder: (context) {
                                        final sizeBytes = e.sizeBytes;
                                        final ticks = e.runTimeTicks ?? 0;
                                        final seconds = ticks > 0
                                            ? ticks / 10000000.0
                                            : 0.0;
                                        final bitrate = sizeBytes != null &&
                                                sizeBytes > 0 &&
                                                seconds > 0
                                            ? ((sizeBytes * 8) / seconds)
                                                .round()
  ...
  Future<void> _reopenStreamWithViewType(VideoViewType next) async {
    ...
    _uiTimer = Timer.periodic(const Duration(milliseconds: 250), (_) {
      ...
      if (!_reportedStop &&
        ...
        if (!_mobilePlaybackEndHandled) {
          _mobilePlaybackEndHandled = true;
          // ignore: unawaited_futures
          _onMobilePlaybackEnded();
        }
        // ignore: unawaited_futures
      ...
      children: [
        if (_isIos) ...[
          MobilePlayerOptionTile(
            title: 'iOS 内核优先级',
            subtitle: '默认优先 AVPlayer，其次 MPV，最后 VLC。切换后会保留当前播放进度。',
            leading: const Icon(
  ...
  Future<void> _init() async {
    ...
    _reportedStop = false;
    _mobilePlaybackEndHandled = false;
    ...
    try {
      ...
      if (initialPrepared != null) {
        ...
        final preloadProxy = preparedProxy.isNotEmpty
            ? preparedProxy
            : PlaybackPreloadCoordinator.resolveHttpProxyUrl(
                appState: widget.appState,
                sourceUrl: initialPrepared.resolvedSource.url,
